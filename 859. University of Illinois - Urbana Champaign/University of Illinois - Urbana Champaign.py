...
MAIN_DOMAIN = 'http://catalog.illinois.edu'
UNIVERSITY = 'University of Illinois - Urbana Champaign'


def get_courses():
    r = requests.get(f'{MAIN_DOMAIN}/courses-of-instruction/', headers=HEADERS)
...
def get_course(url):
    ...
    for tag in div_tags:
        ...
        desc = tag.find('p', class_='courseblockdesc')
        if desc:
            desc = desc.text.strip()

        course_code_1, course_code_2, course_name = strong_tags.text.strip().replace('\xa0', ' ').split(' ', 2)
