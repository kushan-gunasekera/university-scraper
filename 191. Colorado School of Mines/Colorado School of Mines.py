...
MAIN_DOMAIN = 'https://catalog.mines.edu'
...
def get_courses():
    ...
    for tag in course_tags:
        if not tag.get('href') or tag.get('href').startswith('#') or not tag.get('href').startswith('/coursesaz'):
            continue
...
def get_course(url):
    ...
    for tag in div_tags:
        desc = None
        try:
            desc = tag.find('p', 'courseblockdesc').next_sibling.text.strip()
        except:
            pass
