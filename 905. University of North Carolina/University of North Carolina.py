    ...
    try:
        for course_tag in course_tags:
            ...
            if len(strong_tags) == 1:
                print(f'no more than 1 strong tags | {url}')
                continue

            course_code = strong_tags[0].text.strip()
            course_name = strong_tags[1].text.strip()
            courses[course_code] = {
                'course_code': course_code,
                'course_name': course_name
            }
            desc = course_tag.find('p', class_='courseblockextra')
            if desc:
                courses[course_code]['course_description'] = desc.text
