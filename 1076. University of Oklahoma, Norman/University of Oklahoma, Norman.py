...
MAIN_DOMAIN = 'https://ou-public.courseleaf.com'
UNIVERSITY = 'University of Oklahoma, Norman'
...
def get_course(url):
    ...
    for tag in course_tags:
        desc = tag.find('p', 'courseblockdesc').text.strip().replace('\xa0', ' ')
        text = tag.find('strong').text.strip().replace('\xa0', ' ').split('. ', 1)
        courses[text[0]] = {
            'course_code': text[0],
            'course_name': text[1],
