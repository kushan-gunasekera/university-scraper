...
MAIN_DOMAIN = 'https://catalog.wwu.edu'
SUB_DOMAIN_1 = 'https://catalog.wwu.edu/content.php?catoid=21&catoid=21&navoid=6223&filter%5Bitem_type%5D=3&filter%5Bonly_active%5D=1&filter%5B3%5D=1&filter%5Bcpage%5D={page_number}#acalog_template_course_filter'
...
def get_description(page_number, course_code, url):
    ...
    desc = re.sub(r"(Credits:|Prerequisites & Notes:).*", '', desc)
    return {'course_code': course_code, 'description': desc}
...
def main():
    full_courses = {}
    with ThreadPoolExecutor(max_workers=3) as executor:
        for i in as_completed(executor.submit(get_courses, SUB_DOMAIN_1, page_number) for page_number in range(1, 45)):
