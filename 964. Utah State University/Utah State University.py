...
MAIN_DOMAIN = 'https://catalog.unh.edu'
UNIVERSITY = 'Utah State University'


def get_courses():
    r = requests.get(f'{MAIN_DOMAIN}/programs/#view=list', headers=HEADERS)
    soup = BeautifulSoup(r.content, 'html.parser')
    course_tags = soup.find('div', class_='filter-items isotope filters-list').find_all('a')
    ...
    for i in search_urls:
        ...
        for j in course_tags.find_all('article'):
            ...
            if not courseblock:
                continue
            h3_tag = j.find('h3')
            code, title = h3_tag.text.strip().replace('\xa0', ' ').split(' - ', 1)
            desc = []
            try:
                for k in j.find('p',  class_='courseblockdesc noindent').next_siblings:
                    if k.text.startswith(break_values):
                        break
                    desc.append(k.text.strip().replace('\xa0', ' '))
            except:
                pass
            courses[code] = {
                'course_code': code,
                'course_name': title,
                'course_description': ' '.join(desc),
