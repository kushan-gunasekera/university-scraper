...
MAIN_DOMAIN = 'https://catalog.iastate.edu'
UNIVERSITY = 'Iowa State University'


def get_courses():
    r = requests.get(f'{MAIN_DOMAIN}/azcourses/', headers=HEADERS)
...
def get_course(url):
    ...
    # div_tags = soup.find_all('div', class_='courseblocktitle')
    courses = {}
    if not div_tags:
        return courses

    for tag in div_tags:
        desc = tag.find('p', class_='prereq').text
        strong_tags = tag.find('div', class_='courseblocktitle').find('strong')
