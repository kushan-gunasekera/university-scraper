...
MAIN_DOMAIN_1 = 'https://catalog.hope.edu/content.php?catoid=7&catoid=7&navoid=249&filter%5Bitem_type%5D=3&filter%5Bonly_active%5D=1&filter%5B3%5D=1&filter%5Bcpage%5D={page_number}#acalog_template_course_filter'
...
def get_courses(domain, page_number):
    ...
    for tag in course_tags:
        ...
        if len(tag_split) == 2:
            ...
            title = tag_split[1]
            desc = None
            url = f'https://catalog.hope.edu/{tag.get("href")}'
            print(f'description: {code} - {title} | {url} ')
            res = requests.get(url, headers=HEADERS)
            soup = BeautifulSoup(res.content, 'html.parser')
            try:
                desc = soup.find('h1', id='course_preview_title').next_sibling.next_sibling.text.strip()
            except:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        for i in as_completed(executor.submit(get_courses, MAIN_DOMAIN_1, page_number) for page_number in range(1, 16)):
