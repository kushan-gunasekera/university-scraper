...
MAIN_DOMAIN = 'https://catalogue.shu.edu'
...
def get_course(url):
    ...
    for i in search_urls:
        ...
        if not course_tags:
            continue
        for j in course_tags.find_all('div', class_='courseblock'):
            code = j.find('span', class_='text col-2 detail-code margin--tiny text--semibold text--big').text.strip().replace('\xa0', ' ')
            title = j.find('span', class_='text col-7 detail-title margin--tiny text--semibold text--big').text.strip().replace('\xa0', ' ')
            desc = None
            try:
                desc = j.find('p',  class_='courseblockextra noindent').text.strip().replace('\xa0', ' ')
            except:
                pass
            courses[code] = {
                'course_code': code,
