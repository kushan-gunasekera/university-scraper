...
MAIN_DOMAIN_1 = 'https://catalog.calvin.edu/content.php?catoid=22&catoid=22&navoid=707&filter%5Bitem_type%5D=3&filter%5Bonly_active%5D=1&filter%5B3%5D=1&filter%5Bcpage%5D={page_number}#acalog_template_course_filter'
...
def get_courses(domain, page_number):
    ...
    for tag in course_tags:
        ...
        if len(tag_split) == 2:
            ...
            title = tag_split[1]
            desc = None
            url = f'http://catalog.calvin.edu/{tag.get("href")}'
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        for i in as_completed(executor.submit(get_courses, MAIN_DOMAIN_1, page_number) for page_number in range(1, 15)):
