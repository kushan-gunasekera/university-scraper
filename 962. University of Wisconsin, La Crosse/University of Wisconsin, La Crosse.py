...
MAIN_DOMAIN = 'https://catalog.uwlax.edu'
UNIVERSITY = 'University of CA Irvine'


def get_courses(path):
    logging.info('getting urls')
    r = requests.get(f'{MAIN_DOMAIN}{path}', headers=HEADERS)
...
def get_course(url):
    ...
    for tag in course_tags:
        p_tags = tag.find_all('p', class_='courseblocktitle')
        course_code = p_tags[0].text
        course_name = p_tags[1].text
        courses[course_code] = {
