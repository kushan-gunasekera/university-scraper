...
MAIN_DOMAIN = 'https://bulletin.wfu.edu'
...
def get_courses():
    ...
    for tag in course_tags:
        href = tag.get('href')
        if not href or '#' in href or not href.startswith('/courses-az'):
            continue
        courses.append(href)
...
def get_course(url):
    ...
    for tag in course_tags:
        title = tag.find('p', 'courseblocktitle noindent').find('strong').text.strip().replace('\xa0', ' ').split('. ')
