...
MAIN_DOMAIN = 'https://catalog.registrar.uiowa.edu'
UNIVERSITY = 'Missouri University of Science and Technology'


def get_courses():
    logging.info('getting urls')
    r = requests.get(f'{MAIN_DOMAIN}/azdirectory/', headers=HEADERS)
    soup = BeautifulSoup(r.content, 'html.parser')
    course_tags = soup.find('div', id='atozindex').find_all('a')

    courses = []
    for tag in course_tags:
        if not (tag.get('href') and tag.get('href').startswith('/courses/')):
            continue
