...
MAIN_DOMAIN = 'https://catalog.csuchico.edu/'
UNIVERSITY = 'CA State University, Chico'
...
def get_course(url):
    ...
    for tag in course_tags:
        text = tag.find('strong').text.strip().replace('\xa0', ' ')
        courses[text] = {
            'course_code': text,
            'course_name': tag.find_all('span')[1].text.strip().replace('\xa0', ' '),
            'course_description': tag.find('span', class_='text detail-description').text.strip().replace('\xa0', ' '),
        }
