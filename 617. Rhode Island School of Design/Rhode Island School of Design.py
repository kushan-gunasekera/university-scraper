...
MAIN_DOMAIN = 'https://catalog.csudh.edu'
...
def get_course(url):
    ...
    for tag in course_tags:
        code = tag.find('span', 'text col-5 detail-code margin--tiny text--semibold text--huge').text.strip().replace('\xa0', ' ')
        title = tag.find('span', 'text col-5 detail-title margin--tiny text--semibold text--huge').text.strip().replace('\xa0', ' ')
