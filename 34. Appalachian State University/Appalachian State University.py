...
MAIN_DOMAIN_1 = 'https://bulletin.appstate.edu/content.php?catoid=34&catoid=34&navoid=2104&filter%5Bitem_type%5D=3&filter%5Bonly_active%5D=1&filter%5B3%5D=1&filter%5Bcpage%5D={page_number}#acalog_template_course_filter'
MAIN_DOMAIN_2 = 'https://bulletin.appstate.edu/content.php?catoid=33&catoid=33&navoid=2058&filter%5Bitem_type%5D=3&filter%5Bonly_active%5D=1&filter%5B3%5D=1&filter%5Bcpage%5D={page_number}#acalog_template_course_filter'
...
def get_courses(domain, page_number):
    ...
    for tag in course_tags:
        ...
        if len(tag_split) == 2:
            ...
            title = tag_split[1]
            desc = None
            url = f'http://bulletin.appstate.edu/{tag.get("href")}'
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        for i in as_completed(executor.submit(get_courses, MAIN_DOMAIN_1, page_number) for page_number in range(1, 35)):
            full_courses = {**full_courses, **i.result()}

    with ThreadPoolExecutor(max_workers=10) as executor:
        for i in as_completed(executor.submit(get_courses, MAIN_DOMAIN_2, page_number) for page_number in range(1, 17)):
            full_courses = {**full_courses, **i.result()}
