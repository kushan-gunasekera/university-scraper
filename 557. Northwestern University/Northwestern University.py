...
MAIN_DOMAIN = 'https://catalogs.northwestern.edu/'
UNIVERSITY = 'Northwestern University'


def get_courses():
    # print(f'page_number: {page_number}')
    r = requests.get(f'{MAIN_DOMAIN}/tgs/courses-az/', headers=HEADERS)
...
def get_course(url):
    ...
    for tag in course_tags:
        desc = tag.find('p', class_='courseblockdesc').text.strip().replace('\xa0', ' ')
        text = tag.find('p', class_='couresblocktitle').find('strong').text.strip().replace('\xa0', ' ')
        code_1, code_2, title = text.split(' ', 2)
        courses[f'{code_1} {code_2}'] = title
